...
function broadcastEvent(type, data) {
  ...
  sseClients.forEach(client => {
    try {
      ...
      console.warn("Error writing to SSE client:", e.message);
    }
  });
}
...
function readDB() {
  ...
  try {
    ...
    if (!data.settings) data.settings = { passkeys: { food: '9676292195', education: '7671000584', civic: '6301316591', health: '8886803818', others: '7671000583', system: '7671000585' } };
    return data;
  } catch (e) {
...
app.post('/api/complaints', upload.fields([
  ...
  writeDB(db);
  broadcastEvent('new-complaint', newComplaint);
...
const handleStatusUpdate = (req, res) => {
  ...
  writeDB(db);
  broadcastEvent('status-update', complaint);
  res.json(complaint);
};
...
app.post('/api/complaints/resolve', (req, res) => {
  ...
  writeDB(db);
  broadcastEvent('resolve', complaint);
